...
typedef void (*trap_handler_t)(struct pt_regs *regs, uint64_t scause, uint64_t stval);
...
void ecall_from_user_mode_handler(struct pt_regs *regs, uint64_t scause, uint64_t stval) {
    ...
    switch (syscall_nr) {
        ...
        case __NR_clock_gettime: {
            ret = sys_clock_gettime(
                (clockid_t)regs->x[RISCV_REG_A0],
                (struct timespec*)regs->x[RISCV_REG_A1]
            );
            break;
        }
        default: {
...
const struct handler_table {
    ...
    {0, SCAUSE_INST_PAGE_FAULT, FUNC_PTR_TRANS(do_page_fault, trap_handler_t)},
    {0, SCAUSE_LOAD_PAGE_FAULT, FUNC_PTR_TRANS(do_page_fault, trap_handler_t)},
    {0, SCAUSE_STORE_PAGE_FAULT, FUNC_PTR_TRANS(do_page_fault, trap_handler_t)}
};
